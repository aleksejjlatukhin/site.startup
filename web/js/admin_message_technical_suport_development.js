...
$(document).ready(function () {
    ...
    websocket.addEventListener('message', function (response) {
        ...
        if (data.action === 'send-message') {
            ...
            // Обновляем блок с беседами
            if ($(identifyingBlockAdressee).length || identifyingBlockSender.length) {
                $(conversation_list_menu).html(data.conversationsForDevelopmentAjax);
...
setInterval(function(){
    ...
    var url = '/admin/message/get-users-is-online?id=' + conversation_id + '&pathname=technical-support';
