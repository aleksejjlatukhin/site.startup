...
function getQueryResponds() {
    ...
    if ($(window).width() <= '480') {
        if (action === 'view-trash') {
            url = '/responds/get-query-responds?stage=4&id=' + confirmId + '&isMobile=true&isOnlyNotDelete=false';
        } else {
            url = '/responds/get-query-responds?stage=4&id=' + confirmId + '&isMobile=true';
        }
    } else {
        if (action === 'view-trash') {
            url = '/responds/get-query-responds?stage=4&id=' + confirmId + '&isOnlyNotDelete=false';
        } else {
            url = '/responds/get-query-responds?stage=4&id=' + confirmId;
        }
    }
    $.ajax({
        url: url,
...
$(body).on('change', 'input#search_input_responds_mobile', function () {
    ...
    var url = '/responds/get-query-responds?stage=4&id=' + confirmId + '&search=' + search + '&isMobile=true';
