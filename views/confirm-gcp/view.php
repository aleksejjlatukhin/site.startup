...
?>
<div class="confirm-gcp-view table-project-kartik">
    ...
    <div class="tab">
        <?php if ($model->nextStep === false && empty($model->gcps)) : ?>
            <button class="tablinks step_one_button col-xs-12 col-md-6 col-lg-3" onclick="openCity(event, 'step_one')">Шаг 1. Заполнить исходные данные для подтверждения ГЦП</button>
            ...
            <button class="tablinks step_three_button col-xs-12 col-md-6 col-lg-3" onclick="openCity(event, 'step_three')">Шаг 3. Переход к генерации Minimum Viable Product</button>
        ...
        <?php else : ?>
            ...
            <button class="tablinks step_three_button col-xs-12 col-md-6 col-lg-3" onclick="openCity(event, 'step_three')" id="defaultOpen">Шаг 3. Переход к генерации Minimum Viable Product</button>
            <button class="tablinks feedbacks_button col-xs-12 col-md-6 col-lg-3" onclick="openCity(event, 'feedbacks')">Отзывы экспертов</button>
        <?php endif; ?>
    ...
    <div id="step_one" class="tabcontent">
        ...
        echo DetailView::widget([
            ...
            'formOptions' => [
                ...
                'action' => Url::to(['/confirm-gcp/update-data-interview', 'id' => $model->id]),
            ],
            'attributes' => [
                ...
                [
                    ...
                    'columns' => [
                        ...
                        [
                            ...
                            'label' => 'Количество респондентов, подтверждающих ГЦП:',
                            'labelColOptions' => ['class' => 'text-left', 'style' => ['padding' => '10px', 'width' => '40%']],
        ...
        <div class="row" style="display: none;">
            <div class="col-md-12 form-newQuestion" style="margin-top: 5px;">
                ...
                <? $form = ActiveForm::begin(['id' => 'addNewQuestion', 'action' => Url::to(['/confirm-gcp/add-question', 'id' => $model->id])]);?>
        ...
        <div class="row" style="display: none;">
            <div class="col-md-12 form-QuestionsOfGeneralList" style="margin-top: 5px;">
                ...
                <? $form = ActiveForm::begin(['id' => 'addNewQuestionOfGeneralList', 'action' => Url::to(['/confirm-gcp/add-question', 'id' => $model->id])]);?>
        ...
        <div class="row step_two">
            ...
            $gridColumnsResponds_3 = [
                ...
                [
                    ...
                    'label' => 'Вывод о ценностном предложении',
                    'header' => '<div style="padding: 5px; color: #4F4F4F;">Вывод о ценностном предложении</div>',
                    'value' => function($model){

                        if ($model->descInterview->status == 1){
                            return '<div class="text-center" style="color:green">Предложение привлекательно</div>';

                        } elseif($model->descInterview->status == null){
                            return '';

                        } elseif($model->descInterview->status == 0){
                            return '<div class="text-center" style="color:red">Предложение не интересно</div>';
                        }
                    },
                    'format' => 'raw',
                ],
                        ...
                        [
                            ...
                            'value' => function($model){
                                if ($model->descInterview){
                                    return !$model->descInterview->status ? '<span style="color:red">Предложение не интересно</span>' : '<span style="color:green">Предложение привлекательно</span>';
                                }else{
                                    return '';
                                }
    ...
    <div style="font-style: italic"><span class="bolder">Программа подтверждения ГЦП</span> - программа подтверждения гипотезы ценностного предложения.</div>
            ...
            success: function(response){
                ...
                if (!response['error']) {
                    ...
                    if (sumDataExistRespond == 0) {
                        ...
                        $('#messageAboutTheNextStep').html('Начните заполнять анкетные данные респондентов');
                    ...
                    if (sumDataExistRespond == sumResponds && sumDataExistDescInterview == sumResponds && inputCountPositive <= sumDataMembersOfSegment && sumProblems == 0) {
                        ...
                        $('#messageAboutTheNextStep').html('Переходите к генерации Minimum Viable Product');
                        if ($('#messageAboutTheNextStep').hasClass('text-warning')) {
                            $('#messageAboutTheNextStep').removeClass('text-warning').addClass('text-success');
                        }
                        if ($('#messageAboutTheNextStep').hasClass('text-danger')) {
                    ...
                    if (sumDataExistRespond == sumResponds && sumDataExistDescInterview == sumResponds && inputCountPositive > sumDataMembersOfSegment) {
                        ...
                        $('#messageAboutTheNextStep').html('Недостаточное количество респондентов, подтвердивших ценностное предложение');
                        if ($('#messageAboutTheNextStep').hasClass('text-warning')) {
                    ...
                    if (sumProblems == 0 && sumDataExistRespond != 0 &&(sumDataExistRespond != sumResponds || sumDataExistDescInterview != sumResponds)) {
                        ...
                        $('#messageAboutTheNextStep').html('Продолжите заполнение анкетных данных респондентов');
                        if ($('#messageAboutTheNextStep').hasClass('text-danger')) {
