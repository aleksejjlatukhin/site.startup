...
class MessageController extends AppClientController
{
    ...
    public function beforeAction($action)
    {
        ...
        elseif (in_array($action->id, ['index'])) {
            ...
            $adminCompany = User::findOne(['id' => Yii::$app->request->get(), 'role' => User::ROLE_ADMIN_COMPANY]);
            $development = User::findOne(['id' => Yii::$app->request->get(), 'role' => User::ROLE_DEV]);

            // Ограничение доступа
            if ($admin->id == Yii::$app->user->id || $mainAdmin->id == Yii::$app->user->id
                || $development->id == Yii::$app->user->id || $adminCompany->id == Yii::$app->user->id){
                // ОТКЛЮЧАЕМ CSRF
                $this->enableCsrfValidation = false;
                return parent::beforeAction($action);
